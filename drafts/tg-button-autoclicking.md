...
{% hint style="info" %}
Если вы пропустили раздел по начальной подготовке рабочей среды, то самое время с ним ознакомиться по [ссылке](getting-started.md).
{% endhint %}
...
На этом этапе у вас должна быть настроена рабочая среда. Код будет опубликован в репозитории [jpx](https://github.com/johnpantini/jpx). Если вы работаете [по 3-му варианту](../origins/creating-a-repo.md#mode-3-local-clone), просто обновите проект через Ctrl+T и примите изменения в ветке **johnpantini**.
...
{% hint style="danger" %}
На этом этапе посетите раздел про [установку N\|Solid](setting-up-nsolid.md), где посмотрите, как в проект добавить файл **package.json**. Сейчас он нам понадобится!
{% endhint %}
...
### Запуск в RHEL через [N\|Solid](setting-up-nsolid.md)
