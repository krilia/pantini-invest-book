...
{% hint style="info" %}
Запущенный скрипт будет работать без дополнительных действий, пока не будет остановлен внешним воздействием. Чтобы прервать работу запущенного скрипта вручную, нажмите **Ctrl+C**
{% endhint %}
...
В коде нужно лишь поменять ссылку на канал, а также указать другие координаты кнопки при необходимости. Следующий код активирует сигнал на покупку 1 бумаги \(верхняя левая кнопка\):
