...
{% hint style="warning" %}
В данный момент кнопка в виджете PPF не содержит рабочей логики \(обработчика\)
{% endhint %}
...
Несмотря на то, что единственная кнопка в виджете PPF не обрабатывается по нажатию, в ней содержатся данные о цене и направлении заявки. Используя торговый токен, можно повторять заявки на желаемый объем. В примере это будет 1 акция.
